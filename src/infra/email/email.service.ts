...
import { CompanyStatus } from 'src/spvs/schemas/spv.schema';
import { AssetStatus } from 'src/assets/interfaces/asset.type';
...
export class EmailService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!this.brevoApiKey) {
      this.logger.warn(' BREVO_API_KEY is not set in environment variables');
    } else {
  ...
  async sendSpvStatusUpdateEmail(
    email: string,
    context: {
        issuerName: string;
        spvName: string;
    ...
    const subject =
        context.status === CompanyStatus.ACTIVE
        ? 'Your SPV has been Activated'
        : 'Your SPV has been Rejected';
    ...
    async sendAssetStatusUpdateEmail(
        email: string,
        context: {
            issuerName: string;
            assetName: string;
        ...
        const subject =
            context.status === AssetStatus.APPROVED
